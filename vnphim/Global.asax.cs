...
namespace vnphim
{
    public class MvcApplication : System.Web.HttpApplication
    {
        ...
        public static bool IsMobileMode()
        {
            ...
            //v : viewtype | 0 : mobile | 1 : desktop
            if (request.QueryString["v"] != null && request.QueryString["v"].ToString().ToLower() == "0")
            {
                return true;
            }
            else if (request.QueryString["v"] != null && request.QueryString["v"].ToString().ToLower() == "1")
            {
                return false;
            }
            ...
            if (request.Cookies["CK_TGDD_WEB_VERSION"] != null &&
               request.Cookies["CK_TGDD_WEB_VERSION"].Value == "MOBILE")
            {
            ...
            if (request.Cookies["CK_TGDD_WEB_VERSION"] != null &&
                request.Cookies["CK_TGDD_WEB_VERSION"].Value == "DESKTOP")
            {
                if (request.Browser.IsMobileDevice)
                {
                    contextBase.ClearOverriddenBrowser();
        ...
        {
            var request = HttpContext.Current.Request;
            if (request.QueryString["viewtype"] != null && request.QueryString["viewtype"].ToString().ToLower() == "app")
                return true;
