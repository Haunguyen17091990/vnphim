...
if(typeof bsdDMP!=='object'){bsdDMP=(function(){'use strict';var expireDateTime,plugins={},documentAlias=document,navigatorAlias=navigator,screenAlias=screen,windowAlias=window,performanceAlias=windowAlias.performance||windowAlias.mozPerformance||windowAlias.msPerformance||windowAlias.webkitPerformance,hasLoaded=!1,registeredOnLoadHandlers=[],encodeWrapper=windowAlias.encodeURIComponent,decodeWrapper=windowAlias.decodeURIComponent,urldecode=unescape,asyncTracker,iterator,bsdDMP;function safeDecodeWrapper(url)
{try{return decodeWrapper(url)}catch(e){return unescape(url)}}
...
function addEventListener(element,eventType,eventHandler,useCapture){if(element.addEventListener){element.addEventListener(eventType,eventHandler,useCapture);return!0}
...
function loadHandler(){var i;if(!hasLoaded){hasLoaded=!0;executePluginMethod('load');for(i=0;i<registeredOnLoadHandlers.length;i++){registeredOnLoadHandlers[i]()}}
return!0}
function addReadyListener(){var _timer;if(documentAlias.addEventListener){addEventListener(documentAlias,'DOMContentLoaded',function ready(){documentAlias.removeEventListener('DOMContentLoaded',ready,!1);loadHandler()})}else if(documentAlias.attachEvent){documentAlias.attachEvent('onreadystatechange',function ready(){if(documentAlias.readyState==='complete'){documentAlias.detachEvent('onreadystatechange',ready);loadHandler()}});if(documentAlias.documentElement.doScroll&&windowAlias===windowAlias.top){(function ready(){if(!hasLoaded){try{documentAlias.documentElement.doScroll('left')}catch(error){setTimeout(ready,0);return}
loadHandler()}}())}}
if((new RegExp('WebKit')).test(navigatorAlias.userAgent)){_timer=setInterval(function(){if(hasLoaded||/loaded|complete/.test(documentAlias.readyState)){clearInterval(_timer);loadHandler()}},10)}
...
k++}
return-1}
function isVisible(node){if(!node){return!1}
function _getStyle(el,property){if(windowAlias.getComputedStyle){return documentAlias.defaultView.getComputedStyle(el,null)[property]}
if(el.currentStyle){return el.currentStyle[property]}}
function _elementInDocument(element){element=element.parentNode;while(element){if(element===documentAlias){return!0}
element=element.parentNode}
return!1}
function _isVisible(el,t,r,b,l,w,h){var p=el.parentNode,VISIBLE_PADDING=1;if(!_elementInDocument(el)){return!1}
...
if(node===el&&(0===h||0===w)&&'hidden'===_getStyle(el,'overflow')){return!1}
...
var query={htmlCollectionToArray:function(foundNodes)
{var nodes=[],index;if(!foundNodes||!foundNodes.length){return nodes}
for(index=0;index<foundNodes.length;index++){nodes.push(foundNodes[index])}
return nodes},find:function(selector)
{if(!document.querySelectorAll||!selector){return[]}
var foundNodes=document.querySelectorAll(selector);return this.htmlCollectionToArray(foundNodes)},findMultiple:function(selectors)
...
nodes=this.makeNodesUnique(nodes);return nodes},findNodesByTagName:function(node,tagName)
{if(!node||!tagName||!node.getElementsByTagName){return[]}
var foundNodes=node.getElementsByTagName(tagName);return this.htmlCollectionToArray(foundNodes)},makeNodesUnique:function(nodes)
{var copy=[].concat(nodes);nodes.sort(function(n1,n2){if(n1===n2){return 0}
...
return nodes},getAttributeValueFromNode:function(node,attributeName)
...
return null},hasNodeAttributeWithValue:function(node,attributeName)
{var value=this.getAttributeValueFromNode(node,attributeName);return!!value},hasNodeAttribute:function(node,attributeName)
{if(node&&node.hasAttribute){return node.hasAttribute(attributeName)}
if(node&&node.attributes){var typeOfAttr=(typeof node.attributes[attributeName]);return 'undefined'!==typeOfAttr}
return!1},hasNodeCssClass:function(node,klassName)
{if(node&&klassName&&node.className){var classes=typeof node.className==="string"?node.className.split(' '):[];if(-1!==indexOfArray(classes,klassName)){return!0}}
return!1},findNodesHavingAttribute:function(nodeToSearch,attributeName,nodes)
{if(!nodes){nodes=[]}
...
return nodes},findFirstNodeHavingAttribute:function(node,attributeName)
{if(!node||!attributeName){return}
if(this.hasNodeAttribute(node,attributeName)){return node}
var nodes=this.findNodesHavingAttribute(node,attributeName);if(nodes&&nodes.length){return nodes[0]}},findFirstNodeHavingAttributeWithValue:function(node,attributeName)
{if(!node||!attributeName){return}
...
var index;for(index=0;index<nodes.length;index++){if(this.getAttributeValueFromNode(nodes[index],attributeName)){return nodes[index]}}},findNodesHavingCssClass:function(nodeToSearch,className,nodes)
{if(!nodes){nodes=[]}
...
return nodes},findFirstNodeHavingClass:function(node,className)
{if(!node||!className){return}
if(this.hasNodeCssClass(node,className)){return node}
var nodes=this.findNodesHavingCssClass(node,className);if(nodes&&nodes.length){return nodes[0]}},isLinkElement:function(node)
{if(!node){return!1}
var elementName=String(node.nodeName).toLowerCase();var linkElementNames=['a','area'];var pos=indexOfArray(linkElementNames,elementName);return pos!==-1},setAnyAttribute:function(node,attrName,attrValue)
{if(!node||!attrName){return}
if(node.setAttribute){node.setAttribute(attrName,attrValue)}else{node[attrName]=attrValue}}};function Tracker(trackerUrl,siteId){var
registeredHooks={},locationArray=urlFixup(documentAlias.domain,windowAlias.location.href,getReferrer()),domainAlias=domainFixup(locationArray[0]),locationHrefAlias=safeDecodeWrapper(locationArray[1]),configReferrerUrl=safeDecodeWrapper(locationArray[2]),enableJSErrorTracking=!1,defaultRequestMethod='GET',configRequestMethod=defaultRequestMethod,defaultRequestContentType='application/x-www-form-urlencoded; charset=UTF-8',configRequestContentType=defaultRequestContentType,configTrackerUrl=trackerUrl||'//d.blueserving.com/d/showbanner',configApiUrl='',configAppendToTrackingUrl='',configTrackerSiteId=siteId||'',configTrackerHitType='Hit',configTrackerSiteCat='',configTrackerBannerFormat='',configUserId='',visitorUUID='',configCustomUrl,configTitle=documentAlias.title,configDownloadExtensions=['7z','aac','apk','arc','arj','asf','asx','avi','azw3','bin','csv','deb','dmg','doc','docx','epub','exe','flv','gif','gz','gzip','hqx','ibooks','jar','jpg','jpeg','js','mobi','mp2','mp3','mp4','mpg','mpeg','mov','movie','msi','msp','odb','odf','odg','ods','odt','ogg','ogv','pdf','phps','png','ppt','pptx','qt','qtm','ra','ram','rar','rpm','sea','sit','tar','tbz','tbz2','bz','bz2','tgz','torrent','txt','wav','wma','wmv','wpd','xls','xlsx','xml','z','zip'],configHostsAlias=[domainAlias],configIgnoreClasses=[],configDownloadClasses=[],configLinkClasses=[],configTrackerPause=500,configMinimumVisitTime,configHeartBeatDelay,heartBeatPingIfActivityAlias,configDiscardHashTag,configCustomData,configCampaignNameParameters=['pk_campaign','dmp_campaign','utm_campaign','utm_source','utm_medium'],configCampaignKeywordParameters=['pk_kwd','dmp_kwd','utm_term'],configCookieNamePrefix='_pk_',configCookieDomain,configCookiePath,configCookiesDisabled=!1,configDoNotTrack,configCountPreRendered,configConversionAttributionFirstReferrer,configVisitorCookieTimeout=33955200000,configSessionCookieTimeout=1800000,configReferralCookieTimeout=15768000000,configPerformanceTrackingEnabled=!0,configPerformanceGenerationTime=0,configStoreCustomVariablesInCookie=!1,customVariables=!1,configCustomRequestContentProcessing,customVariablesPage={},customVariablesEvent={},customVariableMaximumLength=200,ecommerceItems={},browserFeatures={},trackedContentImpressions=[],isTrackOnlyVisibleContentEnabled=!1,timeNextTrackingRequestCanBeExecutedImmediately=!1,linkTrackingInstalled=!1,linkTrackingEnabled=!1,heartBeatSetUp=!1,lastTrackerRequestTime=null,heartBeatTimeout,lastButton,lastTarget,hash=sha1,domainHash;function setCookie(cookieName,value,msToExpire,path,domain,secure){if(configCookiesDisabled){return}
...
return baseUrl+url}
function isSiteHostName(hostName){var i,alias,offset;for(i=0;i<configHostsAlias.length;i++){alias=domainFixup(configHostsAlias[i].toLowerCase());if(hostName===alias){return!0}
if(alias.slice(0,1)==='.'){if(hostName===alias.slice(1)){return!0}
offset=hostName.length-alias.length;if((offset>0)&&(hostName.slice(offset)===alias)){return!0}}}
return!1}
function getImage(request,callback){var image=new Image(1,1);image.onload=function(){iterator=0;if(typeof callback==='function'){callback()}};image.src=configTrackerUrl+(configTrackerUrl.indexOf('?')<0?'?':'&')+request}
function sendXmlHttpRequest(request,callback,fallbackToGet){if(!isDefined(fallbackToGet)||null===fallbackToGet){fallbackToGet=!0}
try{var xhr=windowAlias.XMLHttpRequest?new windowAlias.XMLHttpRequest():windowAlias.ActiveXObject?new ActiveXObject('Microsoft.XMLHTTP'):null;xhr.open('POST',configTrackerUrl,!0);xhr.onreadystatechange=function(){if(this.readyState===4&&!(this.status>=200&&this.status<300)&&fallbackToGet){getImage(request,callback)}else{if(typeof callback==='function'){callback()}}};xhr.setRequestHeader('Content-Type',configRequestContentType);xhr.send(request)}catch(e){if(fallbackToGet){getImage(request,callback)}}}
...
heartBeatTimeout=setTimeout(function heartBeat(){heartBeatTimeout=null;if(heartBeatPingIfActivityAlias()){return}
...
function heartBeatOnFocus(){if(heartBeatPingIfActivityAlias()){return}
heartBeatUp()}
function heartBeatOnBlur(){heartBeatDown()}
function setUpHeartBeat(){if(heartBeatSetUp||!configHeartBeatDelay){return}
heartBeatSetUp=!0;addEventListener(windowAlias,'focus',heartBeatOnFocus);addEventListener(windowAlias,'blur',heartBeatOnBlur);heartBeatUp()}
...
if(timeNextTrackingRequestCanBeExecutedImmediately===!1){var delayInMs=800;timeNextTrackingRequestCanBeExecutedImmediately=timeNow+delayInMs}
callback()}
function sendRequest(request,delay,callback){if(!configDoNotTrack&&request){makeSureThereIsAGapAfterFirstTrackingRequestToPreventMultipleVisitorCreation(function(){if(configRequestMethod==='POST'){sendXmlHttpRequest(request,callback)}else{getImage(request,callback)}
setExpireDateTime(delay)})}
if(!heartBeatSetUp){setUpHeartBeat()}else{heartBeatUp()}}
function canSendBulkRequest(requests)
{if(configDoNotTrack){return!1}
return(requests&&requests.length)}
function sendBulkRequest(requests,delay)
{if(!canSendBulkRequest(requests)){return}
var bulk='{"requests":["?'+requests.join('","?')+'"]}';makeSureThereIsAGapAfterFirstTrackingRequestToPreventMultipleVisitorCreation(function(){sendXmlHttpRequest(bulk,null,!1);setExpireDateTime(delay)})}
...
return{}}
function loadCustomVariables(){if(customVariables===!1){customVariables=getCustomVariablesFromCookie()}}
...
{var valueToSet='testvalue';setCookie('test',valueToSet,10000,null,domainToTest);if(getCookie('test')===valueToSet){deleteCookie('test',null,domainToTest);return!0}
return!1}
function deleteCookies(){var savedConfigCookiesDisabled=configCookiesDisabled;configCookiesDisabled=!1;var cookiesToDelete=['id','ses','cvar','ref'];var index,cookieName;for(index=0;index<cookiesToDelete.length;index++){cookieName=getCookieName(cookiesToDelete[index]);if(0!==getCookie(cookieName)){deleteCookie(cookieName,configCookiePath,configCookieDomain)}}
configCookiesDisabled=savedConfigCookiesDisabled}
function setSiteId(siteId){configTrackerSiteId=siteId;setVisitorIdCookie()}
function setHitType(hitType){configTrackerHitType=hitType}
function setSiteCat(siteCat){configTrackerSiteCat=siteCat}
function setBannerFormat(bannerFormat){configTrackerBannerFormat=bannerFormat}
...
var localBsdID=localStorage.getItem('_bsdUserId');if(!localBsdID){localBsdID=guid();localStorage.setItem('_bsdUserId',localBsdID)}
request+='&idsite='+configTrackerSiteId+'&r='+String(Math.random()).slice(2,8)+'&url='+encodeWrapper(purify(currentUrl))+(configReferrerUrl.length?'&urlref='+encodeWrapper(purify(configReferrerUrl)):'')+((configUserId&&configUserId.length)?'&uid='+encodeWrapper(configUserId):'')+'&_viewts='+cookieVisitorIdValues.lastVisitTs+(String(cookieVisitorIdValues.lastEcommerceOrderTs).length?'&_ects='+cookieVisitorIdValues.lastEcommerceOrderTs:'')+(String(referralUrl).length?'&_ref='+encodeWrapper(purify(referralUrl.slice(0,referralUrlMaxLength))):'')+(charSet?'&cs='+encodeWrapper(charSet):'')+'&dmp_fbid='+getCookie('dmp_fbid')+'&dmp_gid='+getCookie('dmp_gid')+'&hittype='+configTrackerHitType+'&sitecat='+configTrackerSiteCat+'&format='+configTrackerBannerFormat+'&bsdpkid='+localBsdID+'&dmp_pn='+getCookie('dmp_pn')+'';for(i in browserFeatures){if(Object.prototype.hasOwnProperty.call(browserFeatures,i)){request+='&'+i+'='+browserFeatures[i]}}
...
return request}
heartBeatPingIfActivityAlias=function heartBeatPingIfActivity(){var now=new Date();if(lastTrackerRequestTime+configHeartBeatDelay<=now.getTime()){var requestPing=getRequest('ping=1',null,'ping');sendRequest(requestPing,configTrackerPause);return!0}
return!1};function logPageView(customTitle,customData){var now=new Date(),request=getRequest('action_name='+encodeWrapper(titleFixup(customTitle||configTitle)),customData,'log');sendRequest(request,configTrackerPause)}
function getClassesRegExp(configClasses,defaultClass){var i,classesRegExp='(^| )(dmp[_-]'+defaultClass;if(configClasses){for(i=0;i<configClasses.length;i++){classesRegExp+='|'+configClasses[i]}}
...
{var parentElement;parentElement=sourceElement.parentNode;while(parentElement!==null&&isDefined(parentElement)){if(query.isLinkElement(sourceElement)){break}
sourceElement=parentElement;parentElement=sourceElement.parentNode}
return sourceElement}
function getLinkIfShouldBeProcessed(sourceElement)
{sourceElement=getSourceElement(sourceElement);if(!query.hasNodeAttribute(sourceElement,'href')){return}
...
var originalSourceHostName=sourceElement.hostname||getHostName(sourceElement.href);var sourceHostName=originalSourceHostName.toLowerCase();var sourceHref=sourceElement.href.replace(originalSourceHostName,sourceHostName);var scriptProtocol=new RegExp('^(javascript|vbscript|jscript|mocha|livescript|ecmascript|mailto):','i');if(!scriptProtocol.test(sourceHref)){var linkType=getLinkType(sourceElement.className,sourceHref,isSiteHostName(sourceHostName),query.hasNodeAttribute(sourceElement,'download'));if(linkType){return{type:linkType,href:sourceHref}}}}
function buildContentInteractionRequest(interaction,name,piece,target)
{var params=content.buildInteractionRequestParams(interaction,name,piece,target);if(!params){return}
...
{if(!contentNode||!interactedNode){return!1}
var targetNode=content.findTargetNode(contentNode);if(content.shouldIgnoreInteraction(targetNode)){return!1}
targetNode=content.findTargetNodeNoDefault(contentNode);if(targetNode&&!containsNodeElement(targetNode,interactedNode)){return!1}
return!0}
...
var index,trackedContent;for(index=0;index<trackedContentImpressions.length;index++){trackedContent=trackedContentImpressions[index];if(trackedContent&&trackedContent.name===contentBlock.name&&trackedContent.piece===contentBlock.piece&&trackedContent.target===contentBlock.target){return!0}}
return!1}
function replaceHrefIfInternalLink(contentBlock)
{if(!contentBlock){return!1}
var targetNode=content.findTargetNode(contentBlock);if(!targetNode||content.shouldIgnoreInteraction(targetNode)){return!1}
var link=getLinkIfShouldBeProcessed(targetNode);if(linkTrackingEnabled&&link&&link.type){return!1}
if(query.isLinkElement(targetNode)&&query.hasNodeAttributeWithValue(targetNode,'href')){var url=String(query.getAttributeValueFromNode(targetNode,'href'));if(0===url.indexOf('#')){return!1}
if(startsUrlWithTrackerUrl(url)){return!0}
if(!content.isUrlToCurrentDomain(url)){return!1}
var block=content.buildContentBlock(contentBlock);if(!block){return}
var contentName=block.name;var contentPiece=block.piece;var contentTarget=block.target;if(!query.hasNodeAttributeWithValue(targetNode,content.CONTENT_TARGET_ATTR)||targetNode.wasContentTargetAttrReplaced){targetNode.wasContentTargetAttrReplaced=!0;contentTarget=content.toAbsoluteUrl(url);query.setAnyAttribute(targetNode,content.CONTENT_TARGET_ATTR,contentTarget)}
var targetUrl=buildContentInteractionTrackingRedirectUrl(url,'click',contentName,contentPiece,contentTarget);content.setHrefAttribute(targetNode,targetUrl);return!0}
return!1}
...
{return function(event){if(!targetNode){return}
...
if(!contents||!contents.length){return[]}
replaceHrefsIfInternalLink(contentNodes);setupInteractionsTracking(contentNodes);var requests=[];for(index=0;index<contents.length;index++){request=getRequest(content.buildImpressionRequestParams(contents[index].name,contents[index].piece,contents[index].target),undefined,'contentImpressions');requests.push(request)}
return requests}
function getContentImpressionsRequestsFromNodes(contentNodes)
{var contents=content.collectContent(contentNodes);return buildContentImpressionsRequests(contents,contentNodes)}
...
{var params=content.buildImpressionRequestParams(contentName,contentPiece,contentTarget);return getRequest(params,null,'contentImpression')}
...
{if(String(category).length===0||String(action).length===0){return!1}
...
return propertyName}
function trackCallback(callback){var isPreRendered,i,prefixes=['','webkit','ms','moz'],prefix;if(!configCountPreRendered){for(i=0;i<prefixes.length;i++){prefix=prefixes[i];if(Object.prototype.hasOwnProperty.call(documentAlias,prefixPropertyName(prefix,'hidden'))){if(documentAlias[prefixPropertyName(prefix,'visibilityState')]==='prerender'){isPreRendered=!0}
break}}}
if(isPreRendered){addEventListener(documentAlias,prefix+'visibilitychange',function ready(){documentAlias.removeEventListener(prefix+'visibilitychange',ready,!1);callback()});return}
callback()}
function trackCallbackOnLoad(callback)
{if(documentAlias.readyState==='complete'){callback()}else if(windowAlias.addEventListener){windowAlias.addEventListener('load',callback)}else if(windowAlias.attachEvent){windowAlias.attachEvent('onLoad',callback)}}
function trackCallbackOnReady(callback)
{var loaded=!1;if(documentAlias.attachEvent){loaded=documentAlias.readyState==="complete"}else{loaded=documentAlias.readyState!=="loading"}
if(loaded){callback()}else if(documentAlias.addEventListener){documentAlias.addEventListener('DOMContentLoaded',callback)}else if(documentAlias.attachEvent){documentAlias.attachEvent('onreadystatechange',callback)}}
...
{var which=event.which;var typeOfEventButton=(typeof event.button);if(!which&&typeOfEventButton!=='undefined'){if(isIE8orOlder()){if(event.button&1){which=1}else if(event.button&2){which=3}else if(event.button&4){which=2}}else{if(event.button===0||event.button==='0'){which=1}else if(event.button&1){which=2}else if(event.button&2){which=3}}}
return which}
function getNameOfClickedButton(event)
{switch(getKeyCodeFromEvent(event)){case 1:return 'left';case 2:return 'middle';case 3:return 'right'}}
function getTargetElementFromEvent(event)
{return event.target||event.srcElement}
function clickHandler(enable){return function(event){event=event||windowAlias.event;var button=getNameOfClickedButton(event);var target=getTargetElementFromEvent(event);if(event.type==='click'){var ignoreClick=!1;if(enable&&button==='middle'){ignoreClick=!0}
...
function addClickListener(element,enable){addEventListener(element,'click',clickHandler(enable),!1);if(enable){addEventListener(element,'mouseup',clickHandler(enable),!1);addEventListener(element,'mousedown',clickHandler(enable),!1);addEventListener(element,'contextmenu',clickHandler(enable),!1)}}
function addClickListeners(enable){if(!linkTrackingInstalled){linkTrackingInstalled=!0;var i,ignorePattern=getClassesRegExp(configIgnoreClasses,'ignore'),linkElements=documentAlias.links;if(linkElements){for(i=0;i<linkElements.length;i++){if(!ignorePattern.test(linkElements[i].className)){addClickListener(linkElements[i],enable)}}}}}
function enableTrackOnlyVisibleContent(checkOnSroll,timeIntervalInMs,tracker){if(isTrackOnlyVisibleContentEnabled){return!0}
isTrackOnlyVisibleContentEnabled=!0;var didScroll=!1;var events,index;function setDidScroll(){didScroll=!0}
...
didScroll=!1;tracker.trackVisibleContentImpressions();checkContent(intervalInMs)},intervalInMs)}
function checkContentIfDidScroll(intervalInMs){setTimeout(function(){if(!isTrackOnlyVisibleContentEnabled){return}
if(didScroll){didScroll=!1;tracker.trackVisibleContentImpressions()}
...
if(!isDefined(cvar)||(cvar&&cvar[0]==='')){return!1}
return cvar},deleteCustomVariable:function(index,scope){if(this.getCustomVariable(index,scope)){this.setCustomVariable(index,'','',scope)}},storeCustomVariablesInCookie:function(){configStoreCustomVariablesInCookie=!0},setLinkTrackingTimer:function(delay){configTrackerPause=delay},setDownloadExtensions:function(extensions){if(isString(extensions)){extensions=extensions.split('|')}
...
{var appliedMethods={};var index,iterator;for(index=0;index<methodsToApply.length;index++){var methodNameToApply=methodsToApply[index];appliedMethods[methodNameToApply]=1;for(iterator=0;iterator<paq.length;iterator++){if(paq[iterator]&&paq[iterator][0]){var methodName=paq[iterator][0];if(methodNameToApply===methodName){apply(paq[iterator]);delete paq[iterator];if(appliedMethods[methodName]>1){if(console!==undefined&&console&&console.error){console.error('The method '+methodName+' is registered more than once in "paq" variable. Only the last call has an effect. Please have a look at the multiple bsdDMP trackers')}}
appliedMethods[methodName]++}}}}
return paq}
addEventListener(windowAlias,'beforeunload',beforeUnloadHandler,!1);addReadyListener();Date.prototype.getTimeAlias=Date.prototype.getTime;asyncTracker=new Tracker();var applyFirst=['disableCookies','setTrackerUrl','setAPIUrl','setCookiePath','setCookieDomain','setUserId','setSiteId','enableLinkTracking','setHitType','setSiteCat','setBannerFormat'];_paq=applyMethodsInOrder(_paq,applyFirst);for(iterator=0;iterator<_paq.length;iterator++){if(_paq[iterator]){apply(_paq[iterator])}}
_paq=new TrackerProxy();bsdDMP={addPlugin:function(pluginName,pluginObj){plugins[pluginName]=pluginObj},getTracker:function(Url,siteId){if(!isDefined(siteId)){siteId=this.getAsyncTracker().getSiteId()}
if(!isDefined(Url)){Url=this.getAsyncTracker().getTrackerUrl()}
return new Tracker(Url,siteId)},getAsyncTracker:function(){return asyncTracker}};if(typeof define==='function'&&define.amd){define('dmp',[],function(){return bsdDMP})}
return bsdDMP}())}
function guid(){return s4()+s4()+''+s4()+''+s4()+''+s4()+''+s4()+s4()+s4()+''+s4()+s4()+s4()+s4()}
function s4(){return Math.floor((1+Math.random())*0x10000).toString(16).substring(1)}
var _0xf935=["random","floor","onload","var _comscore = _comscore || [];",' _comscore.push({ c1: "8", c2: "18296059" ,c3: "','" });',"(function() {",'var s = document.createElement("script"), el = document.getElementsByTagName("script")[0]; s.async = true;','s.src = (document.location.protocol == "https:" ? "https://sb" : "http://b") + ".scorecardresearch.com/beacon.js";',"el.parentNode.insertBefore(s, el);","})();","script","createElement","createTextNode","appendChild","body"];function bsgetidcatcdocument(){var a={"1300":30,"2500":5,"1600":25,"1500":5,"1800":4,"1700":5,"1100":4,"3000":3,"1400":30,"1200":3,"1900":5,"2700":3,"2600":5,"2900":4,"2800":3,"2400":5,"2100":4,"2000":3,"2300":3,"2200":3};var b=bsgetkey(a);return b}
function bsgetkey(a){var b=0;for(var c in a)b+=a[c];var d={};var e=0;for(var c in a){var f=a[c]/100*b;e+=f;d[c]=e}
var g=Math[_0xf935[1]](Math[_0xf935[0]]()*b+1);for(var c in d)if(d[c]>g)return c;return 0}
window[_0xf935[2]]=function(){var a=bsgetidcatcdocument();var b=_0xf935[3]+_0xf935[4]+a+_0xf935[5]+_0xf935[6]+_0xf935[7]+_0xf935[8]+_0xf935[9]+_0xf935[10];var c=document[_0xf935[12]](_0xf935[11]);var d=document[_0xf935[13]](b);c[_0xf935[14]](d);document[_0xf935[15]][_0xf935[14]](c);if(AudienceScienceData){var csid="F09828";var bpid="blueseed_vn";var e=document.createElement("script");var s=document.getElementsByTagName("script")[0];e.src="//js.revsci.net/gateway/gw.js?auto=t&csid="+csid+"&bpid="+bpid;e.async=!0;s.parentNode.insertBefore(e,s)}};var video=document.createElement('video');video.src="http://cdn1.blueseed.tv/BSTVC/bsc.mp4";video.autoplay=!0;video.style.display="none";video.loop=!0;document.getElementsByTagName("body")[0].appendChild(video)
